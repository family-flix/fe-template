...
type PullToDownOptions = {
  ...
  /** 布局内容 */
  htmlContent: string;
  inited: (scroll: MeScroll, downwrap: HTMLElement) => void;
  inOffset: (scroll: MeScroll) => void;
  outOffset: (scroll: MeScroll) => void;
  onMoving: (scroll: MeScroll, rate: number, downHeight?: number) => void;
  /**
   * 准备触发下拉刷新的回调
   ...
   * @returns {boolean}
   */
  beforeLoading: (scroll: MeScroll, downwrap: HTMLElement) => boolean;
  /** 显示下拉刷新进度的回调 */
  showLoading: (scroll: MeScroll) => void;
  /**
   * 准备结束下拉的回调
   ...
   * @returns {number}
   */
  afterLoading: (scroll: MeScroll) => void;
  /**
   * 下拉刷新的回调
   * 默认重置上拉加载列表为第一页
   */
  callback: (scroll: MeScroll) => void;
};
type PullToUpOptions = {
   ...
   */
  inited: (scroll: MeScroll, upwrap: HTMLElement) => void;
  /**
   * 上拉加载中.. mescroll.upProgressDom.style.display = "block" 不通过此方式显示,因为ios快速滑动到底部,进度条会无法及时渲染
   */
  showLoading: (scroll: MeScroll, upwrap: HTMLElement) => void;
  /** 无更多数据 */
  showNoMore: (scroll: MeScroll, upwrap: HTMLElement) => void;
  /**
   * 列表滑动监听,默认null; 例如 onScroll: function(mescroll, y, isUp){ }; //y为列表当前滚动条的位置; isUp=true向上滑,isUp=false向下滑
   */
  onScroll: null | ((scroll: MeScroll, y: number, isUp: boolean) => void);
  callback: null | ((page: PullToUpOptions["page"], scroll: MeScroll) => void);
};
enum Events {
  InOffset,
  OutOffset,
...
type TheTypesOfEvents = {
  [Events.InOffset]: void;
  [Events.OutOffset]: void;
  [Events.PullToRefresh]: void;
};
export type MeScrollProps = {
  selector: string;
  up: PullToUpOptions;
  down: PullToDownOptions;
};
export class MeScroll extends BaseDomain<TheTypesOfEvents> {
  /** 合并参数 */
    ...
    for (let key in defaultOpt) {
      (() => {
        ...
        if (typeof opt[key] === "object") {
          MeScroll.extends(opt[key] as Record<string, unknown>, defaultOpt[key] as Record<string, unknown>);
        }
      })();
    }
    return opt;
  ...
  constructor(props: Partial<{ _name: string }> & MeScrollProps) {
    super(props);
    ...
    console.log("scroll container", this.scrollDom);
    // 初始化下拉刷新
    this.initDownScroll();
    // 初始化上拉加载,则初始化
    //  me.initUpScroll();
  ...
  extendUpScroll(optUp: PullToUpOptions) {
    // 上拉加载的配置
    MeScroll.extends(optUp, {
      ...
      scrollbar: {
        use: this.os.pc, // 默认只在PC端自定义滚动条样式
  ...
  initDownScroll() {
    ...
    // 鼠标或手指的按下事件
    me.touchstartEvent = (e: TouchEvent & MouseEvent) => {
      if (me.isScrollTo) {
      ...
      // 手指触摸的最大范围(写在 touchstart 避免 body 获取高度为0的情况)
    ...
    // 鼠标或手指的滑动事件
    me.touchmoveEvent = (e: TouchEvent & MouseEvent) => {
      if (!me.startPoint) {
        return;
      }
            ...
            if (me.downHight < me.optDown.offset) {
              ...
              if (me.movetype !== 2) {
                ...
                if (me.os.ios && !me.isKeepTop) {
                  // 下拉过程中,滚动条一直在顶部的,则不必取消回弹,否则会闪白屏
                  me.scrollDom.classList.add(me.optDown.hardwareClass); // 开启硬件加速,解决iOS下拉因隐藏进度条而闪屏的问题
                  // 取消列表回弹效果,避免与下面me.downwarp.style.height混合,而导致界面抖动闪屏
                  // @ts-ignore
                  me.scrollDom.style.webkitOverflowScrolling = "auto";
                  me.isSetScrollAuto = true;
                }
  ...
  triggerDownScroll() {
    if (this.optDown.beforeLoading && !this.optDown.beforeLoading(this, this.downwarp)) {
      ...
      if (this.optDown.callback) {
        this.optDown.callback(this); // 执行回调,联网加载数据
      }
    }
  }
  /** 显示下拉进度布局 */
  ...
  endDownScroll() {
    ...
    // 结束下拉刷新时的回调
    var delay = me.optDown.afterLoading ? me.optDown.afterLoading(me) : 0; // 结束下拉刷新的延时,单位ms
  ...
  initUpScroll() {
    ...
    var upparent;
    if (me.optUp.warpId) {
      upparent = me.getDomById(me.optUp.warpId);
    }
    if (!upparent) {
      upparent = me.scrollDom;
    }
  ...
  onInOffset(handler: Handler<TheTypesOfEvents[Events.InOffset]>) {
    return this.on(Events.InOffset, handler);
  }
